...
def MiniMaxGame(board, color, depth):
    if depth == 0:
        nodes = 1
    ...
    for position in boardpositions:
        if color == 'W':
            newboard, nodes, newvalue = MiniMaxGame(list(position), 'B', depth - 1)
            nodecount += nodes
            if newvalue > value:
                value = newvalue
                bestboard = position
        else:
            newboard, nodes, newvalue = MiniMaxGame(list(position), 'W', depth - 1)
            nodecount += nodes
            if newvalue < value:
                value = newvalue
                bestboard = position
...
if __name__ == '__main__':
    ...
    nextboard, poscount, val = MiniMaxGame(input_board, 'W', int(sys.argv[3]))
